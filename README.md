...
1.2.30 针对“点击鸡哥说话时 Codex 主窗口也弹出”加入独立点击接收层，并修复后台窗口拖拽过程中不跟随鼠标、松开后才跳动的问题。它只覆盖鸡哥人物，不覆盖旁边的控制按钮；拖拽时会将输入交给 Codex 并持续转发移动。运行助手仍保持独立后台启动。

隔离桌面中的 Chromium 验证了点击屏蔽及移动窗口时的连续拖拽；Windows PowerShell 5.1 已通过脚本编译检查。本机使用者确认单击说话不唤起 Codex，拖拽途中连续跟随、松开不跳动。跨屏切换和第三方截图软件配合尚未单独验收。具体变更见 [CHANGELOG.md](CHANGELOG.md)。
